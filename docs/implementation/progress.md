...
| MANAGER-25 | RC5 | S8 | audit-2026-09-21 | open (second fresh-eyes audit, lead-verified by reading. `shutdown_all()` → `_stop_then_teardown` runs **stop and teardown per device, sequentially**, so anything that holds the GUI thread inside one device's `close()` — a slow drain, or PYSIDE-21's modal — leaves every later-registered device **not yet stopped**. Registration order puts the Temperature Controller before the SMC100 Rotator. `full_stop_all()` already avoids exactly this by fanning out with a bounded join; shutdown does not. **Fix shape:** stop every device first (reuse the bounded fan-out), then tear down. Safety path, lead-owned) |
