...
| MANAGER-21 | RC5 | S8 | audit-2026-09-21 | open (**`SystemManager.full_stop_all` reports a stop confirmed when it cannot know that.** `_stop` sets `ok = True` the instant `model.emergency_stop()` returns without raising (system_manager.py:261-267). But every model's `emergency_stop` is built to *always* return inside `ESTOP_RETURN_BUDGET` (0.08 s) whether or not the hardware write landed — on expiry it `print()`s "latched; hardware stop still in flight" and returns normally. It never raises and has no way to say "unconfirmed". The method's own docstring — "A model reported as False is a model whose stop did not confirm in time... its hardware write is still in flight" — therefore describes behaviour the code cannot produce; `False` is reachable only if `emergency_stop` blocks past `FULL_STOP_BUDGET` (1.0 s), which by construction it does not. The Web frontend renders this as "FULL STOP confirmed for all devices" while an axis may still be moving. **Compounds with ROTATOR-16**, which was the concrete way the write could fail to land. Fixing it is a contract change — `emergency_stop` must report whether the write confirmed — across `base.py`'s `ManagedModel`, all three models, `system_manager.py` and the Web adapter. Safety path, lead-owned, not delegable. The existing tests miss it because they mock `emergency_stop` to raise or hand-return `False`; none exercises the real shape, a model returning successfully but unconfirmed. Found by a fresh-eyes audit 2026-09-21, verified by the lead) |
| MANAGER-22 | RC5 | S8 | audit-2026-09-21 | open (**a latched FULL STOP cannot be cleared from any frontend.** `clear_estop()` is the only sanctioned way to release `_estop` and has **zero callers**: `grep -rn "clear_estop|estop_latched" src/` returns only the three definitions and their three properties. No view, no `ui_schema` button, no Web endpoint. Once latched, `_refuse_if_estopped` blocks every transition for the rest of that model's life (probes.py:708). **This is not a rare corner.** D-8's client-liveness watchdog calls `self.emergency_stop()` automatically when no Web client has polled for `WEB_CLIENT_STOP_TIMEOUT` while in AUTONOMOUS or MANUAL (probes.py:1231-1241) — and that timeout is still the **unmeasured 15 s placeholder** WEB-19 leaves open. So a Web operator who alt-tabs, sleeps a laptop, or loses the network for 15 s mid-jog gets that probe latched, and the only recoveries are restarting the process — dropping every other device's connection — or re-running the whole setup wizard, which is the initial hardware wizard, not a resume control. RC-5 calls `clear_estop` "explicit operator action", which presupposes a control that was never built. **Bench-blocking; the single highest-value item outstanding.** Found by a fresh-eyes audit 2026-09-21, chain verified end to end by the lead) |
