...
| PYSIDE-21 | RC8 | S11 | audit-2026-09-21 | closed (confirmed, not just hypothesised: `QtErrorPopupManager._message_signal` used AutoConnection, which Qt resolves to a *direct* call when the publisher is on the GUI thread, so a `requires_ack` error raised inside `closeEvent → shutdown_all()` ran `QMessageBox.critical()` — a nested event loop — before `report_error()` returned. Now connected with `Qt.QueuedConnection`; worker-thread delivery is unchanged, since AutoConnection already queued those. test_requires_ack_modal_does_not_run_before_publish_returns (lead mutation check: fails with the plain connect restored), test_requires_ack_modal_still_runs_from_a_worker_thread — both qt, run by the lead on merge, qt 64 → 69. MANAGER-25 was the model-side half and closed first, so even a modal that did open could no longer leave a device unstopped) |
...
| SERIAL-23 | RC2 | S3 | audit-2026-09-21 | open (partly closed: **the host no longer puts a second write on the wire while one is in flight**, which was the precondition for a swallowed `'d'`. The fix is TEMP-17's `_write_io_lock`, taken by `send_manual_mode_command`'s raw packet write as well, so a priority disable now waits for the 42-byte packet to finish and lands after it. test_serial23_a_priority_disable_cannot_land_inside_a_manual_packet (overlap on b46a734; none after). The misleading "a mangled *stop* is the only thing this risks" comment in `write_command` is corrected. That removes the bench question as it was asked — whether two unsynchronised pyserial writes interleave no longer matters, because the host no longer issues them. **Open on:** the one case left is a writer wedged inside `ser.write` past `WRITE_IO_LOCK_TIMEOUT`, where the stop still goes regardless; a partial packet followed by `'d'` would still be swallowed by `readBytes`. Closing that needs framing in the firmware — D-7 scope) |
...
| TEMP-17 | RC2 | S3 | audit-2026-09-21 | open (partly closed: **host half closed, and it was worse than the audit said.** Two defects, one root — the transport's priority path, after `PRIORITY_LOCK_TIMEOUT`, wrote with no synchronisation at all. (1) *Interleave*: `_lock` spans whole transactions including the heater reader's blocking `readline()`, so forcing is routine, and a forced frame could overlap a write already in `ser.write` — a split `<...>` frame is what reaches `atof(NULL)`. (2) *Order inversion*, found by the lead while pinning (1): an Enter Settings that passed its FULL STOP check waits inside `write_command` for `_lock` behind the reader; the stop forces its zero frame through; the reader releases; the stale `<80,...>` frame is written **after** the stop and the heater re-arms — reproduced on b46a734 for both `emergency_stop()` and a plain Stop System. Fix: `serial._write_io_lock`, innermost, held only around `ser.write` at all four write sites, which a forced write still waits for (bounded by `WRITE_IO_LOCK_TIMEOUT` = 0.25 s, so a wedged writer cannot hold a stop); and `write_command(abort_if=...)`, evaluated at the wire, with the heater's `_stop_generation` (bumped first thing in every `stop()`) so a superseded settings frame is dropped and reported. test_temp17_a_forced_priority_write_waits_out_an_in_flight_write, test_temp17_a_settings_frame_waiting_behind_the_reader_cannot_land_after_a_stop[emergency_stop], [stop], test_temp17_a_wedged_writer_still_cannot_hold_a_stop_for_long, test_temp17_abort_if_is_checked_at_the_write_and_skips_it — all fail on b46a734. **Open on:** the firmware's missing NULL checks in `parseData` — D-7 scope, owner-only) |
