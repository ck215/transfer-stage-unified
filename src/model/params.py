...
def table(*params):
    """Build a name -> Param mapping, rejecting duplicates loudly."""
    result = {}
    for param in params:
        if param.name in result:
            raise ValueError(f"duplicate param {param.name!r}")
        result[param.name] = param
    return result


def extend(base, *params):
    """A subclass's table: the base's parameters with these overriding."""
    result = dict(base)
    for param in params:
        result[param.name] = param
    return result
