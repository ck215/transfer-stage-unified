"""Capture every byte the OLD code (`legacy/src/`) puts on the wire, as JSON.
...
beside it. `tests/test_wire_golden.py` then (a) re-runs this (in a
subprocess, `python3 tests/golden/capture.py <file>.json`) and asserts the
JSON is current, and (b) replays the same scenarios against the
new `src/` classes and compares byte for byte.
...
_LEGACY_SRC = os.path.join(_ROOT, "legacy", "src")


def _use_legacy_src():
    """Put the old code on `sys.path`. Called by the capture entry point
    only, never at import: the old `legacy/src/` and the new `src/` both have
    top-level `model`/`controller`/`views` packages and can never share a
    process, and the replay tests import this module for `FILES` and
    `HANDSHAKE_PAYLOADS` alongside the new code."""
...
def capture_file(filename):
    ...
    return {
        "_meta": {
            "description": description,
            "captured_from": "legacy/src/ (the old code), driven in simulator mode",
            "regenerate_with": "python3 tests/golden/capture.py",
...
def print_capture(filename):
    """Print one file's capture as JSON on stdout, and nothing else there:
    the old code prints its own chatter (destructors included, at exit), so
    `sys.stdout` points at stderr for the rest of the process."""
    real_stdout = sys.stdout
    sys.stdout = sys.stderr
    document = capture_file(filename)
