...
AWAITING = {
    "probe": "awaiting src/model/probe.py implementation",
    "heater": "awaiting src/model/heater.py implementation",
    "smc100": "awaiting src/devices/smc100.py and "
              "src/model/rotator.py implementation",
}
...
def test_every_scenario_names_its_new_class_call():
    """Each golden scenario documents what it maps to in `src/`.
...
def _build_probe(device_name, port):
    """A new `model.probe` instance wired to `port`.
    ...
    """
    module = importlib.import_module("model.probe")
...
def _build_heater(port):
    module = importlib.import_module("model.heater")
