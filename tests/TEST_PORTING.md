> **Paths after the relayout.** `tests/…` paths below now mean `legacy/tests/…` (the old suite), except `tests/station/…`, which is now `tests/…`.
> `station/models/X` → `src/model/X`; `station/model.py` → `src/model/base.py`; `station/controller.py|setup.py` → `src/controller/`;
> every other `station/X` → `src/X`.
