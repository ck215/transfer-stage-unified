...
class DCView:
    def __init__(self, root: tk.Tk, model: DCModel, controller):
        self.root = root
        self.model = model
    ...
    def _build_ui(self):
        ...
        for axis, var in [("X", self.model.pos_x), ("Y", self.model.pos_y), ("Z", self.model.pos_z)]:
            tk.Label(self.root, text=f"{axis} Position:").grid(row=row_counter, column=0, padx=5, pady=2, sticky='w')
            tk.Label(self.root, textvariable=var, font=('Arial', 10, 'bold'), fg='blue').grid(row=row_counter, column=1, padx=5, pady=2, sticky='w')
            row_counter += 1
        ...
        for axis, var in [("X", self.model.x_step), ("Y", self.model.y_step), ("Z", self.model.z_step)]:
            tk.Label(self.root, text=f"{axis} Step Size:").grid(row=row_counter, column=0, padx=5, pady=2, sticky='w')
        ...
        for axis, var in [("X", self.model.x_dist), ("Y", self.model.y_dist), ("Z", self.model.z_dist)]:
            tk.Label(self.root, text=f"{axis} Steps:").grid(row=row_counter, column=0, padx=5, pady=2, sticky='w')
            tk.Entry(self.root, textvariable=var).grid(row=row_counter, column=1, padx=5, pady=2)
            row_counter += 1
        ...
        tk.Entry(self.root, textvariable=self.model.slow_speed).grid(row=row_counter, column=1, padx=5, pady=2); row_counter += 1
        
        tk.Label(self.root, text="Brake Distance (Counts):").grid(row=row_counter, column=0, padx=5, pady=2, sticky='w')
        tk.Entry(self.root, textvariable=self.model.brake_distance).grid(row=row_counter, column=1, padx=5, pady=2); row_counter += 1
        ...
        self.manual_mode_button = tk.Button(self.root, text="ENTER MANUAL MODE", bg='darkblue', fg='black', font=('Arial', 10, 'bold'), command=self.controller.enter_manual_mode_button)
